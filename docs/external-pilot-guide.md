...
默认按“最少信息”原则交流：报告者先发第 8 节模板；只有维护者说明为什么需要更多字段、通过什么渠道传输，并获得报告者明确同意后，才扩大材料范围。是否同意公开匿名摘要不影响参与试运行。

本手册是可选反馈入口，不是 `0.1.0` 发布或发布后暂缓功能开发的前置条件。报告者无需为了满足项目路线而安装额外 Agent、登录账号或修改系统设置。
