...
- 运行：`python -m agent_preflight scan`、`agent-preflight scan --json`
- 针对性测试：`python -m pytest`
- 完整测试：`python -m pytest && python -m ruff check .`
- 构建或检查：`python -m ruff check .`
- 清理缓存：本阶段不建设项目缓存；pytest/ruff 生成的缓存可直接删除。

## 项目约定

- 目录职责：`src/win_agent_preflight/models.py` 数据模型；`runner.py` 外部命令边界；`windows.py` Windows 事实采集；`checks.py` 诊断分类；`reporting.py` 输出；`cli.py` 参数与编排。
