...
- 当前阶段：首组严格案例、外部试运行手册和 `0.1.0` 发布整理已完成；唯一发布顺序是维护者先确定日期并定稿 0.1.0 Changelog、提交并推送最终发布材料、等待该提交 Windows CI 成功、在同一提交创建 `v0.1.0` tag/Release 并上传已验制品，之后暂缓功能扩展，维护与反馈响应继续。外部试运行是可选反馈入口，不是发布或恢复开发的前置条件。
